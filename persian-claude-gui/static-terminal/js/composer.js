/* ============================================================================
   ...
   A FACTORY since MA3-T1: one per cell. What is left at module level is what
   belongs to the WINDOW rather than to a conversation — the away clock (there
   is one person, and one of them is either here or not), the key sheet, and the
   two verb tables, which are data plus a `cell` parameter.

   The pickers are no longer imported: they live on the cell now
   (`cell.controls`), which also retires composer.js -> controls.js.
   ========================================================================= */
"use strict";
...
/* `!` and `/bash`, one call. Module level so ARG_VERBS below can reach it; the
   only thing it needs from the cell is which conversation the command runs in,
   and `tab` is explicit because a tab-less POST routes by the server's active
   tab — which is not necessarily the cell the user typed into (MA3 §2). */
async function runBash(cell, command) {
  try {
...
const LIFECYCLE_VERBS = {
  model: (cell) => cell.controls.openModelPicker(),
...
/* --- one composer per cell -------------------------------------------------
   ...
   up inside its own `root` (MA3 §1). `cell` is read at CALL time, never
   cached — `cell.tab` changes under this closure every time the view is
   switched, and every session-scoped POST below has to carry the CURRENT one.

   spec-test.html passes `root = document`: the harness IS the cell. */
export function makeComposer(root, cell) {

  // Cell-local elements (T0): `id` -> `class` inside #stage. Window-level ids
  // (keys dialog, new-chat button) stay `getElementById`.
  ...
  function checkIdle(now = Date.now()) {
    ...
    // `body`, not this cell: the class describes whichever conversation
    // agents.js last painted for, and that is always the focused one. Which is
    // why app.js tickIdle() asks the focused column and no other.
  ...
  const hsRow = $("history-search");
  const hsMatch = $("hs-match");
  let searchOn = false;
  let searchDraft = "";
  let searchMatches = [];
  let searchIndex = 0;
  ...
  function promptKeys(e) {
    ...
    switch (chord) {
      ...
      case "alt+p":
        ...
        if (!cell.controls.openModelPicker()) return;
        e.preventDefault();
        return;
      default:
        break;
  ...
  function interceptLifecycle(text) {
    ...
    if (open) return arg ? false : open(cell) !== false;
    if (ARG_VERBS[verb]) return ARG_VERBS[verb](cell, arg) !== false;
    return runWindowCommand(verb, arg, cell);
  }
  ...
  function initComposer() {
    ...
    composer.addEventListener("submit", async (e) => {
      ...
      if (command !== null) {
        ...
        refreshBashMode();
        endHistoryWalk();
        if (command) runBash(cell, command);
        return;
      }
    ...
    /* The two DOCUMENT-level chords this file used to bind — Esc (stop the
       running turn) and Shift+Tab (cycle the approval posture) — moved to
       app.js at MA3-T2 and are registered ONCE for the window, then dispatched
       to the focused cell. Bound here they would have been registered N times,
       once per column, and every one of them would have acted on its own
    ...
    /* Drag-and-drop. TP1 took the paperclip off the prompt (a terminal prompt
       has no buttons), so this and Ctrl+V are the two ways in that V2-PLAN
       §3.2 lists — and only one of them was built. A dropped File and a pasted
       one are the same object, so this is the SAME route, not a second one.

       `dragover` must preventDefault or the browser refuses the drop, and
       `drop` must preventDefault whenever files are on it or Chromium
       NAVIGATES the window to the dropped file, taking the conversation with
       it. */
    const hasFiles = (dt) => [...(dt?.items ?? [])].some((i) => i.kind === "file");
    ...
    /* The idle tick (a minute-long interval plus a visibilitychange listener)
       ...
       outlive a cell `/split 1` removed and go on poking a detached column. */
    ...
    for (const [cls, key] of [["hs-label", "searchLabel"], ["hs-hint", "searchHint"]]) {
      const el = $(cls);
      if (el) el.textContent = FA[key];
    }
    for (const [id, key] of [["keys-title", "keysTitle"], ["keys-close", "keysClose"],
  ...
  /* What the rest of the window is allowed to reach. `input` is the element the
     key gates elsewhere test focus against; everything else is a verb app.js and
     render.js used to import by name. */
  return {
    input,
