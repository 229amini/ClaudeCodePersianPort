...
/* --- one control set per cell ----------------------------------------------
   ...
   looked up inside its own `root` (MA3 §1); snapshot/restore stay, because a
...
export function makeControls(root, cell) {
  ...
  // Cell-local elements (T0): `id` -> `class` inside the cell's own root (and
  // in spec-test.html's stub, which keeps its ids too).
  ...
  const ui = {
    /* v2.4: one inline dialog, in the flow above the prompt, for every picker —
       ...
       for them: three of its assertions prove a background tab's model never
       leaks into the visible one, and they read the label this module writes.
       Every paint below is element-optional, so index.html simply has nothing
       to paint. */
    picker: $("picker"),
  ...
  /* The verbs the rest of the window used to import by name. `pickerOpen` and
     `closePicker` are here because the key gates ask this cell whether ITS
