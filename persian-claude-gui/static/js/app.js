/* ============================================================================
   ...
   Module map (was one 1100-line classic script until 2026-08-05):
     ...
     controls.js  makeControls: pickers + approval pill, per cell
   ...
   THE CELL. Three of those modules are FACTORIES (MA4-T1): a cell is one
   ...
   it is currently showing. MA4-T2 builds 1, 2 or 4 of them into #grid from
   ...
   THE TWO WINDOW-LEVEL CHORDS (Esc, Shift+Tab), Alt+1..4 and the idle clock
   are registered ONCE, at the foot of this file, and dispatched to the FOCUSED
   cell. Bound inside a factory they would have been registered per column, and
   one Esc would have stopped four turns.
...
/* THE CELLS (MA4-T2). 1, 2 or 4 columns, stamped from index.html's
   <template id="cell-tpl"> into #grid. `cells` is exported for spec-test.html
   and test_layout.py, which have no grid at all and ARE the one cell
   (`root = document.body`), reaching this window's control set through it. */
export const cells = [];

/* Which cell the keyboard belongs to. An INDEX, not a reference: cells are
   created and removed by the split control, and every caller asks rather than
   holding a pointer that a shrink would leave dangling. */
let focused = 0;
...
/* `body.busy` is still written -- agents.js and the idle hint read it -- but
   with a grid it can only mean ONE column, and the honest one is the column the
...
/* ONE conversation's worth of window. `tab` is written by placeIn() below and
   read by every session-scoped request the three factories make -- never
   cached by them, because it changes here. */
function makeCell(root) {
  const cell = {
    ...
    // somewhere to write, and focus can move onto an empty column without
    // `state` still describing the one it left.
    blankScope: null,
...
/* One more column, from the template index.html carries. Cell order is DOM
   order and the page is dir="rtl", so cell «۱» is the TOP-RIGHT one -- which
   is what the badge in each topbar is for. */
function addCell() {
...
/* `/split 1|2|4` (js/composer.js) and the window bar's segmented control, and
   nothing else. Shrinking PARKS the removed columns' tabs -- the server tab
   stays open and the sidebar still lists it, because a layout control must
   never close a conversation -- and hands any permission those columns were
   asking to a dialog that still exists. */
...
/* --- the window bar's segmented control -------------------------------------

   The web edition is a window for someone who will never type a command, so
   the split has to be visible and clickable rather than a chord. It is built
   here, not in index.html, for the reason every other Persian label in this
   shell is: the digits are formatted and the titles come from strings.fa.js.
...
/* The untagged path is the original one and stays exactly as it was: it is what
   ...
   render (the agents strip, the sidebar) means "the conversation the keyboard
   is in", and withRenderTarget puts `state` back the way it found it. Every
   other tab -- placed in another column or parked -- is rendered with `state`
   pointed at its own scope for the length of the call. */
...
/* No conversation in this column: it was closed, or its tab moved to another
   column. The column itself stays on screen and shows the home state. */
function blank(cell) {
  if (!cell) return;
...
/* This file used to hold a second copy of the apply table, and the two drifted.
   It is render.js APPLY now -- one table, whether the fact arrived live (on
   screen) or was parked while the tab was in the background. */
...
/* The server decides which tab a tab-less endpoint routes to -- so the POST
   goes first and a refusal (that tab is gone) switches nothing. A tab already
   on screen is not re-placed: the keyboard moves to its column instead, which
   is the whole of "a tab lives in at most one cell". */
...
setTabBridge({
  ...
  // MA4-T2: which column the keyboard is in, all of them, and the layout verb
...
/* The first column. Here rather than at the top of the file for the reason the
   ...
   root (an ELEMENT, so `root.classList` and `.querySelector` both work). */
if (document.getElementById("grid")) {
  addCell();
  paintSplitControl(1);
} else {
  cells.push(makeCell(document.body));
}
...
/* --- the window's own keys --------------------------------------------------
   ...
   Three chords and one clock, one registration each, dispatched to the FOCUSED
   column. They used to be bound inside makeComposer(), which was correct while
   there was one composer and wrong the moment there were four (MA4-T2). */
...
/* Shift+Tab cycles the approval posture, as it does in the TUI — the TUI has no
   ...
   point. The rule is inverted from the obvious one: the composer's own textarea
   cycles, everything editable or focus-trapping does not, and the inert parts of
   the page (body, the transcript) fall through to the cycle. A closed
...
/* Esc stops the running turn, the way it does in the TUI. Routed through the
   ...
   asked of the CELL for the things a cell has one of each of (its dialog, its
   two popups) and of the DOCUMENT for the things the window has one of (a
   popover). `defaultPrevented` covers whatever claims the key after this was
   written (the sidebar's rename field already does). Idle Esc stays unbound on
   purpose: the TUI clears the box with it, and here the box holds the only copy
   of what was typed. */
document.addEventListener("keydown", (e) => {
...
/* Alt+1..4 moves the keyboard between columns. `e.code`, never `e.key`: a
   Persian keyboard layout puts «۱» in `e.key` and the chord would never
   match. */
...
/* The idle hint's clock: ONE per window rather than one per column (an interval
   bound in a factory would outlive a column a later `/split 1` removes). The
   minute tick catches a window that never lost focus; visibilitychange makes
   the return itself instant instead of up-to-a-minute late.

   THE FOCUSED COLUMN ONLY. The hint's own gate is `body.agents-running`
   (agents.js), and that class describes the conversation the strip was last
   painted for -- the focused one, because refreshAgents() is gated to it
   (render.js onFocused). Ticking all four columns would ask three of them a
   question about somebody else's helpers, which is exactly the misread the
   gate was added for. A column the keyboard is not in is judged when the
   keyboard arrives. */
