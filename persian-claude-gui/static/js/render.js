...
const APPLY = {
  ...
  // ponytail: the subagent label map is module-global (setAgents below), so
  // this one key is a window write rather than a cell write. A per-cell map
  // would be a second registry for a tooltip.
...
/* Built here rather than in index.html, exactly like the background-agents
   ...
   conversation. One per CELL (MA4-T1), stored on it. */
...
/* A `!` line is not in the event stream at all: the wrapper parks its tagged
   output and it rides in FRONT of the next message as its own text block
   (server.py bash_message / park_context), which is the TUI's own tagging and
   the shape the CLI writes for its own `!` lines too. So a REPLAYED transcript
   delivers the command and its output as prose inside the user's bubble, tags
   and all. Measured 2026-09-05 in ~/.claude/projects: `<bash-input>CMD
   </bash-input>` followed by an optional `<bash-stdout>…</bash-stdout>` and an
   optional `<bash-stderr>…</bash-stderr>`, in that order.

   The exit code is nowhere in the transcript, so a replayed row reports none —
   `code: 0` is "say nothing about it", which is what renderShell already does
   for a command that worked. */
function splitBashBlocks(text) {
  // Built per call: a /g regex carries lastIndex, and one shared between two
  // messages skips the second one's first block.
  const re = /<bash-input>([\s\S]*?)<\/bash-input>(?:\s*<bash-stdout>([\s\S]*?)<\/bash-stdout>)?(?:\s*<bash-stderr>([\s\S]*?)<\/bash-stderr>)?/g;
  const shells = [];
  const rest = text.replace(re, (_all, command, stdout, stderr) => {
    shells.push({ command: command.trim(), stdout: stdout ?? "",
    ...
    case "user": {
      ...
      for (const part of content) {
        ...
        if (part.type === "text") {
          ...
          // one either, which is what makes the two views converge.
          const bash = splitBashBlocks(part.text ?? "");
          for (const shell of bash.shells) renderShell(shell);
          // Whatever was NOT a bash block is still the person's message; a
          // block that carried nothing else leaves no bubble at all.
          const said = bash.shells.length ? bash.rest : (part.text ?? "");
          if (bash.shells.length && !said) continue;
